...
def dualHorn(expr, i):
    ...
    return match_1_arg(expr,
            [
            (lambda expr: expr["type"] in ["const", "var", "neg"]  ,  lambda expr: dualHorn_same(expr, i)),

            (lambda expr: expr["type"] is "and"       ,     lambda expr: dualHorn_and(expr, i)),
            (lambda expr: expr["type"] is "or"        ,     lambda expr: dualHorn_or(expr, i))
            ])

def dualHorn_same(expr, i):
    return expr

# and 2 similar
def dualHorn_and(expr, i):

    TF1 = expr["expr1"]
    TF2 = expr["expr2"]
    # TF1 = dualHorn(expr["expr1"], i)
    # TF2 = dualHorn(expr["expr2"], i)

    print ("\n--------------------- " + str(i))
    print ("current expr:" + print_expr(expr))
    print ("TF1:" + print_expr(TF1))
    print ("TF2:" + print_expr(TF2))
    print ("---\n")
    ...
    if TF2["type"] == "or":
        ...
        if TF1["type"] == "or":
            ...
            if F21 == F11:
                print ("F11:" + print_expr(F21))
                print ("F21:" + print_expr(F11))
                F4 = dualHorn(mk_and_expr(F22, F12), i)
                F5 = mk_or_expr(F21, F4)
                print ("F5_11: " + print_expr(F5))
                return F5
            if F21 == F12:
                print ("F11:" + print_expr(F21))
                print ("F21:" + print_expr(F12))
                F4 = dualHorn(mk_and_expr(F22, F11), i)
                F5 = mk_or_expr(F21, F4)
                print ("F5_12: " + print_expr(F5))
                return F5
            if F22 == F12:
                print ("F11:" + print_expr(F22))
                print ("F21:" + print_expr(F12))
                F4 = dualHorn(mk_and_expr(F21, F11), i)
                F5 = mk_or_expr(F22, F4)
                print ("F5_13: " + print_expr(F5))
                return F5
            if F22 == F11:
                print ("F11:" + print_expr(F22))
                print ("F21:" + print_expr(F11))
                F4 = dualHorn(mk_and_expr(F21, F12), i)
                F5 = mk_or_expr(F22, F4)
                print ("F5_14: " + print_expr(F5))
                return F5

        if TF1["type"] == "and":
            F11 = TF1["expr1"]
            F12 = TF1["expr2"]
            print ("conditions: TF1 is a & expression")
            print ("F11: " + print_expr(F11))
            print ("F12: " + print_expr(F12))
            if F12["type"] == "or":
                F4 = dualHorn(mk_and_expr(TF2, F12), i)
                return mk_and_expr(F11, F4)
            if F11["type"] == "or":
                F4 = dualHorn(mk_and_expr(TF2, F11), i)
                return mk_and_expr(F12, F4)

    else:
        print ("TF2 is not an _or_:" + print_expr(TF2))
        return mk_and_expr(TF1, TF2)

    # if TF2["type"] == "and":
    #     F21 = TF2["expr1"]
    #     F22 = TF2["expr2"]
    #     if F21["type"] == "or":
    #         F4 = dualHorn(mk_and_expr(F21, TF1))
    #         return dualHorn(mk_and_expr(F22, F4))
    #     if F22["type"] == "or":
    #         F4 = dualHorn(mk_and_expr(F22, TF1))
    #         return dualHorn(mk_and_expr(F21, F4))
    ...
    #         F21 = (TF2["expr1"])
    #         F22 = (TF2["expr2"])
    #         # print ("TF1_11: " + print_expr(TF1_11))
    #         # print ("F21: " + print_expr(F21))
    #         if F21 == TF1_11:
    #             F4 = dualHorn(mk_and_expr(F22, TF1_12))
    #             F5 = mk_or_expr(F21, F4)
    #             print ("F5_1:" + print_expr(F5))
    #             return F5
    ...
    #         # print ("F21: " + print_expr(F21))
    #         if F21 == TF1_21:
    #             F4 = mk_and_expr(F22, TF1_22)
    #             F5 = dualHorn(mk_or_expr(F21, F4))
    #             print ("F5_2:" + print_expr(F5))
    #             return F5
    # else:
    #     F11 = (TF1["expr1"])
    #     F12 = (TF1["expr2"])
    ...
    #     # print ("F21:" + print_expr(F21))
    #     if F11 == F21:
    #         F4 = mk_and_expr(F12, F22)
    #         F5 = mk_or_expr(F11, F4)
    #         print ("F5_3:" + print_expr(F5))
    #         return F5

    return mk_and_expr(TF1, TF2)

def dualHorn_or(expr, i):
...
def cnf_list(expr):
    ...
    if expr["type"] == "and":
        L = cnf_list(expr["expr1"])
        L1 = cnf_list(expr["expr2"])
        if type(L[0]) == list and type(L1[0]) == list:
            L.extend(L1)
            return L
        if type(L[0]) == list:
            L.append(L1)
